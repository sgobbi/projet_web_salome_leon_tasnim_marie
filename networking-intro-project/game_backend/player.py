...
class Player:
    ...
    # fonctions pour le compte des accessoires
    def get_life(self):
        return self._life
    ...
    def move(self, dx, dy, map):
        ...
        if map[new_y][new_x] == "." or map[new_y][new_x] == "x" :
            ...
            # si le joueur a plus d'une epee, il peut tuer le monstre. Sinon il meurt.
            if self._swords >=1:
                ret = True
                map[new_y][new_x] = self._symbol
                map[self._y][self._x] = "."
                ...
                self._life -= 1
                if self._life == 0:
                    map[self._y][self._x] = '.' # le joueur meurt s'il n'a plus de vies.
